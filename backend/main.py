...
def readiness_check():
    ...
    try:
        connection_stats = manager.get_connection_stats()
        checks["websocket_manager"] = {"status": "healthy", "message": "WebSocket manager operational"}
    except Exception as e:
        checks["websocket_manager"] = {"status": "unhealthy", "message": f"WebSocket manager error: {str(e)}"}
        all_healthy = False
...
@app.websocket("/api/v1/connections/ws/{agent_id}")
