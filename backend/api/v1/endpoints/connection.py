...
def create_connection(
    ...
    try:
        ...
        logger.info(f"Created new connection: {new_connection.name} (ID: {new_connection.id}) with API key and WebSocket URL")
...
async def refresh_schema(
    ...
    try:
        ...
        if not manager.is_agent_connected(agent_id):
            raise HTTPException(
                status_code=503, 
                detail=f"Agent '{agent_id}' is not currently connected"
            )
        ...
        if not response or response.get("status") != "success":
            ...
            raise HTTPException(
                status_code=502, 
                detail=f"Agent failed to process schema request: {error_detail}"
            )
